...
def prepare_costs(networks_dict):
...
def to_csv(df, run_name):
    df.to_csv(f"workflow/results/{run_name}/csvs/EU_costs.csv")


def plot_costs(run_name, config):
    cost_df = pd.read_csv(
        f"workflow/results/{run_name}/csvs/EU_costs.csv", index_col=list(range(3)), header=list(range(4))
    )
    ...
    fig.savefig(f"workflow/results/{run_name}/EU_balances/EU_costs.svg", bbox_inches="tight")
    plt.close(fig)
...
costs = prepare_costs(networks_dict)

to_csv(costs, run_name)

plot_costs(run_name, config)
